...
void addMember(const char *roomName) {
  ...
  if (fgets(member.memberAddress, sizeof(member.memberAddress), stdin) ==
      NULL) {
  ...
  if (fwrite(roomName, sizeof(char), MAX_ROOM_NAME, memberRec) !=
      MAX_ROOM_NAME) {
...
void removeMember() {
  ...
  while (fread(currentRoom, sizeof(char), MAX_ROOM_NAME, memberRec) ==
         MAX_ROOM_NAME) {
    ...
    if (strcmp(currentRoom, roomName) == 0) {
      ...
      if (member.id == idNumber) {
        ...
        case 1:
          ...
          if (fgets(member.memberName, sizeof(member.memberName), stdin) ==
              NULL) {
        ...
        case 4:
          ...
          if (fgets(member.memberAddress, sizeof(member.memberAddress),
                    stdin) == NULL) {
...
void viewMembers(const char *roomName) {
  ...
  while (fread(fileRoomName, sizeof(char), MAX_ROOM_NAME, memberRec) ==
         MAX_ROOM_NAME) {
    ...
    printf("ID: %d, Name: %s, Age: %d, Sex: %s, Address: %s, Phone: %d\n",
           member.id, member.memberName, member.age, member.sex,
           member.memberAddress, member.phoneNumber);
  }
